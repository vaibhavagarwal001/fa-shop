...
use App\Models\Floweraura\Nodes;
...
/**Models */
use App\Models\Floweraura\PlViewUrlMapping;
...
class ProductsController extends Controller
{
     ...
     * 
     */
    public function getListingCount($nodeIds)
    {
        $Nodes =  Nodes::join('uc_products', function ($join) {
            $join->on('uc_products.nid', '=', 'node.nid');
        })->join('field_data_uc_product_image', function ($join) {
            $join->on('field_data_uc_product_image.entity_id', '=', 'node.nid');
        })->join('file_managed', function ($join) {
            $join->on('file_managed.fid', '=', 'field_data_uc_product_image.uc_product_image_fid');
        })->join('url_alias', function ($join) {
            $join->on('url_alias.source', '=', DB::raw("CONCAT('node/', node.nid)"));
        })
        ->groupBy("node.nid")
        ->select("node.nid")
        ->whereIn("node.nid" , $nodeIds)
    ...
    public function getlisting(Request $request , $currentPage = 1)
    {
        try {
            $path = $request->path();
            $path = str_replace("api/floweraura/","",$path);
            
            $UrlMapping = PlViewUrlMapping::join('pl_view_product_mapping', function ($join) {
            ...
            $totalRecords = $this->getListingCount($nodeIds);
            $limit = 10;
            ...
            $Nodes = Nodes::join('uc_products', function ($join) {
                ...
                ->select("node.nid", "node.title" , "uc_products.sell_price" , DB::raw("GROUP_CONCAT(file_managed.uri) as uri") , "url_alias.alias")
                ->orderBy("node.nid", "asc")
                ->whereIn("node.nid" , $nodeIds)
                ->limit($limit)
                ->offset($offset)
            ...
            if ($Nodes->first()) {
                ...
                foreach ($Nodes as $node) {
                    $uris = explode(",",$node->uri);
                    foreach($uris as $key => $uri){
                        $uris[$key] = Config('constant.FLOWERAURA_IMAGE_BASE_URL'). str_replace("public:", "", str_replace("/", "", $uri)); 
                    }
                    $temp[] = [
                        "node_id" => $node->nid,
                        "title" => $node->title,
                        "url" => $node->alias,
                        "amount" => (int) $node->sell_price,
                        "image_url" => $uris                        
                    ];
                }
                $data = $temp;
